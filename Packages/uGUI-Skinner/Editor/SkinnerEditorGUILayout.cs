...
namespace Pspkurara.UI.Skinner
{
	...
	public static class SkinnerEditorGUILayout
	{
		...
		{
			IntField(label, property, int.MinValue, int.MaxValue, options);
		}

		/// <summary>
		/// <see cref="EditorGUILayout.IntField"/>を表示 (最小・最大値でClampする)
		/// </summary>
		/// <param name="label">ラベル</param>
		/// <param name="property">プロパティ</param>
		/// <param name="minValue">最小の値</param>
		/// <param name="maxValue">最大の値</param>
		/// <param name="options">レイアウト設定</param>
		public static void IntField(GUIContent label, SerializedProperty property, int minValue, int maxValue, params GUILayoutOption[] options)
		{
			bool showMixedValue = EditorGUI.showMixedValue;
			if (property.hasMultipleDifferentValues)
			{
